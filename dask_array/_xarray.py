...
def _ensure_registered() -> None:
    ...
    _patch_dask_array_compat()


def _patch_dask_array_compat() -> None:
    """Route xarray's dask-specific helpers back through this package.

    Some xarray paths bypass the chunk-manager API and call
    ``xarray.compat.dask_array_compat`` directly when an input satisfies
    ``is_duck_dask_array``. ``dask_array.Array`` intentionally satisfies that
    predicate, so patch those hard-coded legacy-dask helpers at registration
    time for our array type.
    """
    try:
    ...
    original = compat.sliding_window_view
    if getattr(original, "_dask_array_expr_patched", False):
        return

    array_cls = DaskArrayExprManager().array_cls

    def sliding_window_view(x, window_shape, axis=None, *, automatic_rechunk=True, **kwargs):
        if isinstance(x, array_cls):
            import dask_array as da

            return da.sliding_window_view(
                x,
    ...
    sliding_window_view._dask_array_expr_patched = True
    compat.sliding_window_view = sliding_window_view
