"""Tests for ``dask_array.fft``.

Ported from upstream ``dask/array/tests/test_fft.py`` (dask 2025.12.0) with
project-local adaptations: ``import dask_array as da``, ``assert_eq`` and
``same_keys`` from ``dask_array._test_utils``, ``normalize_chunks`` from
``dask_array._core_utils``, and module-level test arrays built directly
(upstream guards them behind ``_array_expr_enabled``).
"""
...
def test_fft_n_kwarg(funcname):
    ...
    assert_eq(da_fft(darr2, 12, axis=0, norm="ortho"), np_fft(nparr, 12, axis=0, norm="ortho"))
    assert_eq(
...
def test_scipy_fftpack_future_warning(modname):
    ...
    if modname == "scipy.fftpack":
        # Check that a FutureWarning is raised when using scipy.fftpack with
        # allow_fftpack=False
        with pytest.warns(FutureWarning, match="does not match NumPy's API and is considered legacy"):
