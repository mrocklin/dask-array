...
| [testing.md](./testing.md) | Writing or fixing tests. Using `assert_eq` (not `np.testing.assert_array_equal`). Testing optimizations with `._name` comparison and `.simplify()`. Parametrization with `pytest.mark.parametrize`. Test invocation (canonical commands live in `docs/development.md`). |
| [block-info-layout-contract.md](./block-info-layout-contract.md) | Touching `map_blocks` `block_info`/`block_id`, `ChunksFreeze`, or any rewrite that changes a subtree's chunk layout. Why the layout a consumer constructed against is pinned rather than late-binding the payloads to the optimized layout. |
